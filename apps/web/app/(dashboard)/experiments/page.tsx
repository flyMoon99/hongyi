...
import { mockExperiments, mockCustomers } from '@/lib/mock-data'
...
export default function ExperimentsPage() {
  ...
  const filtered = experiments.filter(
    (e) =>
      ...
      e.powerEquipment.includes(search) ||
      e.contactPerson.includes(search),
  )
  ...
  const handleSave = async (data: any) => {
    ...
    if (editing) {
      setExperiments((prev) =>
        prev.map((e) =>
          e.id === editing.id
            ? { ...e, ...data, customerName: customer?.companyName, updatedAt: new Date().toISOString() }
            : e,
        ),
      )
  ...
  const handleDelete = async () => {
    if (!deleting) return
    setExperiments((prev) => prev.filter((e) => e.id !== deleting.id))
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">共 {experiments.length} 条试验计划</p>
        </div>
        ...
        <CardContent className="p-0">
          ...
          <Table>
            ...
            <TableBody>
              {filtered.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center text-slate-400 py-12">暂无试验数据</TableCell>
                </TableRow>
              ) : (
                filtered.map((item) => {
      ...
      <ExperimentForm open={formOpen} onClose={() => setFormOpen(false)} onSave={handleSave} experiment={editing} isSaving={isSaving} />
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              确定要删除 <strong>{deleting?.customerName}</strong> 的试验记录吗？此操作不可撤销。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
