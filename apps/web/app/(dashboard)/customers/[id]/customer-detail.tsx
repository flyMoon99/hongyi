...
import { mockCustomers, mockInspections, mockExperiments, mockCustomerLogs } from '@/lib/mock-data'
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <Button variant="outline" asChild>
          <Link href="/customers"><ArrowLeft size={14} className="mr-2" />返回列表</Link>
        </Button>
      </div>
    )
  }

  const inspections = mockInspections.filter((i) => i.customerId === id).slice(0, 5)
  const experiments = mockExperiments.filter((e) => e.customerId === id).slice(0, 5)
  const logs = mockCustomerLogs.filter((l) => l.customerId === id).slice(0, 10)
    ...
    <div className="space-y-5 max-w-5xl">
      ...
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="sm" asChild className="text-slate-500">
          <Link href="/customers"><ArrowLeft size={14} className="mr-1" />返回</Link>
        </Button>
        <div>
            ...
            {inspections.length === 0 ? (
              ...
              <div className="space-y-2">
                {inspections.map((item) => (
                  <div key={item.id} className="flex items-start justify-between p-3 rounded-lg bg-slate-50 hover:bg-slate-100 transition-colors">
                    ...
                    <Badge variant={item.frequency === 'MONTHLY' ? 'info' : 'success'} className="text-xs shrink-0 ml-2">
                  ...
                  <div key={item.id} className="flex items-start justify-between p-3 rounded-lg bg-slate-50 hover:bg-slate-100 transition-colors">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <span className="text-xs text-slate-400">下次：{formatDate(item.nextTestDate) || '暂无'}</span>
                      </div>
                    </div>
                    <Badge variant={item.frequency === 'MONTHLY' ? 'info' : 'success'} className="text-xs shrink-0 ml-2">
                      {item.frequency === 'MONTHLY' ? '月度' : '季度'}
                  ...
                  <div key={log.id} className="relative">
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-1">操作人：{log.operatorName || log.operatorId}</p>
                      </div>
                      <span className="text-xs text-slate-400 shrink-0 ml-4">{formatDateTime(log.createdAt)}</span>
                    </div>
                  </div>
                ))}
              </div>
